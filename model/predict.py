...
# Build the exact same MobileNet structure you just trained in your terminal
...
# Load your newly trained 10.2MB weights file safely
...
def run_inference(img_path):
    ...
    if img is None:
        raise ValueError("Could not read incoming image.")
        
    img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    img_resized = cv2.resize(img_rgb, (224, 224))
    
    # Standardize pixel arrays to match your model's expectations
    img_tensor = img_resized.transpose((2, 0, 1)).astype(np.float32) / 255.0
    mean = np.array([0.485, 0.456, 0.406]).reshape((3, 1, 1))
    std = np.array([0.229, 0.224, 0.225]).reshape((3, 1, 1))
    img_tensor = (img_tensor - mean) / std
    
    torch_tensor = torch.from_numpy(img_tensor).unsqueeze(0).to(device)
